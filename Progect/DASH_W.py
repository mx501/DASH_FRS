...
if geo == "h":
    ...
    PUT_SET_to = "C:\\Users\\lebedevvv\\Desktop\\Показатели ФРС\\ЧЕКИ\\2023\\"
...
class NEW:
    ...
    def Check_set(self):
        ...
        for file in file_paths:
            set_01 = pd.read_excel(file, parse_dates=["Дата/Время чека"], date_format="%d.%m.%Y %H:%M:%S" )
            set_check = set_01[["Тип","Магазин 1C","Магазин","Дата/Время чека","Касса","Чек", 'Наименование товара', "Стоимость позиции","Код товара"]]
            del set_01
            gc.collect()
            # фильтрация таблицы продаж
            set_check = set_check.loc[set_check["Тип"] == "Продажа"]
            ...
            for x in PODAROK:
                set_check = set_check[~set_check['Наименование товара'].str.contains(x)]
            # удаление столбца номенклатура
            set_check = set_check.drop('Наименование товара', axis=1)
            MEMORY().mem_total(x="Закгрузка файла чеков")
